...
function stars(r) {
	...
	if (r['loveid'] != null) {
		$(".vod-sect #wish").hide();
		$(".vod-sect .sect-show").show();
	} else {
		$(".vod-sect #wish").show();
		$(".vod-sect .sect-show").hide();
	}
	if (r['remindid'] != null) {
		$(".vod-rss #dingyue").hide();
		$(".vod-rss .sect-show").show();
	} else {
		$(".vod-rss #dingyue").show();
		$(".vod-rss .sect-show").hide();
	}
	if (curstars > 0) {
...
$("#indexlogin,#indexreg").click(function() {
	$("#indexlogin,#indexreg").attr('src', Root +"index.php?s=user-login-verify-" + Math.random());
});	
